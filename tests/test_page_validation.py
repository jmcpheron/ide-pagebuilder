...
class TestPageDefinitions:
    ...
    def test_model_view_structure(self, page_files):
        ...
        for page_file in page_files:
            ...
            assert 'components' in model_view, f"modelView missing 'components' in {page_file}"
            assert isinstance(model_view['components'], list), f"components must be a list in {page_file}"
    
    def test_component_types(self, page_files):
        """Test that components have valid types."""
        valid_types = ['literal', 'block', 'resource', 'data', 'select', 'input', 'button']
        
        def check_components(components, file_path):
            for component in components:
                if 'type' in component:
                    assert component['type'] in valid_types, f"Invalid component type '{component['type']}' in {file_path}"
        ...
        def check_literals(components, file_path):
            for component in components:
                if component.get('type') == 'literal':
                    assert 'value' in component, f"Literal component missing 'value' field in {file_path}"
        ...
        def check_resources(components, file_path):
            for component in components:
                if component.get('type') == 'resource':
                    assert 'resource' in component, f"Resource component missing 'resource' field in {file_path}"
                    resource_name = component['resource']
                    # Basic format check - should start with "virtualDomains."
                    assert resource_name.startswith('virtualDomains.'), f"Invalid resource reference '{resource_name}' in {file_path}"
        ...
        def check_data_components(components, file_path):
            for component in components:
                if component.get('type') == 'data':
                    assert 'model' in component, f"Data component missing 'model' field in {file_path}"
    ...
    def test_constant_name_matches_filename(self, page_files):
        ...
        for page_file in page_files:
            ...
            assert constant_name == expected_name, f"constantName '{constant_name}' doesn't match filename '{expected_name}' in {page_file}"