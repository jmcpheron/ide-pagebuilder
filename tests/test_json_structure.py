...
class TestJSONStructure:
    ...
    def json_files(self):
        ...
        for json_file in Path('.').rglob('*.json'):
            ...
            if any(part in json_file.parts for part in ['.venv', 'venv', 'node_modules']):
                continue
    ...
    def test_component_schema(self):
        ...
        def validate_components(components, file_path):
            for component in components:
                try:
                    # Basic type check
                    assert isinstance(component, dict), f"Component must be object in {file_path}"
    ...
    def test_extraction_map_schema(self):
            ...
            "properties": {
                ...
                "literals": {
                    ...
                    "items": {
                        ...
                        "required": ["component_path", "name", "filename", "content_hash"]
                    }
                }
            },
        ...
        for map_file in Path('.').rglob('_extraction_map.json'):
            try:
                ...
                pytest.fail(f"Extraction map schema validation failed for {map_file}: {e}")
            except json.JSONDecodeError:
        ...
        def check_duplicate_names(components, file_path, path="root"):
            ...
            for i, component in enumerate(components):
                if 'name' in component:
                    name = component['name']
                    if name in names:
                        pytest.fail(f"Duplicate component name '{name}' found at {path} in {file_path}")
                    names.append(name)
