                ...
                if matches:
                    ...
                    for match in matches:
                        if not any(safe in match.lower() for safe in [
                            'placeholder', 'example', 'test', 'demo', 'xxx', '***'
                        ]):
                            filtered_matches.append(match)
                    
                    if filtered_matches:
                        pytest.fail(f"Potential hardcoded secret found in {page_file}: {filtered_matches}")
    ...
    def test_no_dangerous_javascript(self, page_files):
        ...
        def check_content_for_dangerous_patterns(content, file_path):
            for pattern in dangerous_patterns:
                ...
                if matches:
                    ...
                    if filtered_matches:
                        pytest.fail(f"Potentially dangerous JavaScript pattern found in {file_path}: {filtered_matches}")
        ...
        def check_components_for_dangerous_js(components, file_path):
            for component in components:
                ...
                if 'components' in component:
                    check_components_for_dangerous_js(component['components'], file_path)
        ...
        for page_file in page_files:
            ...
            if 'modelView' in data and 'components' in data['modelView']:
                check_components_for_dangerous_js(data['modelView']['components'], page_file)
    ...
    def test_sql_injection_protection(self, page_files):
        ...
        def check_for_sql_injection(content, file_path):
            for pattern in sql_patterns:
                ...
                if matches:
                    ...
                    if safe_matches:  
                        pytest.fail(f"Potential SQL injection vulnerability in {file_path}: {safe_matches}")
        ...
        def check_components_for_sql_injection(components, file_path):
            for component in components:
                ...
                # Recursively check nested components
                if 'components' in component:
                    check_components_for_sql_injection(component['components'], file_path)
        ...
        for page_file in page_files:
            ...
            if 'modelView' in data and 'components' in data['modelView']:
                check_components_for_sql_injection(data['modelView']['components'], page_file)
    
    def test_external_resource_domains(self, page_files):
        """Test that external resources are from trusted domains (optional check)."""
        # Skip CDN testing as requested by user
        pytest.skip("CDN testing disabled per user request")
    ...
    def test_access_control_fields(self, page_files):
        ...
        for page_file in page_files:
            ...
            if 'developerSecurity' in data:
                assert isinstance(data['developerSecurity'], list), f"developerSecurity must be a list in {page_file}"
            
            # If there are pageRoles, they should be objects or strings
            if 'pageRoles' in data:
                assert isinstance(data['pageRoles'], list), f"pageRoles must be a list in {page_file}"
                for role in data['pageRoles']:
                    assert isinstance(role, (str, dict)), f"pageRoles must contain strings or objects in {page_file}"
                    if isinstance(role, dict):
                        assert 'roleName' in role, f"pageRole objects must have roleName in {page_file}"
    ...
    def test_no_sensitive_data_in_comments(self, page_files):
        ...
        def check_comments_for_sensitive_data(content, file_path):
            for pattern in comment_patterns:
                ...
                if matches:
                    ...
                    if filtered_matches:
                        pytest.fail(f"Potentially sensitive data in comments in {file_path}: {filtered_matches}")
        ...
        def check_components_for_sensitive_comments(components, file_path):
            for component in components:
                ...
                # Recursively check nested components
                if 'components' in component:
                    check_components_for_sensitive_comments(component['components'], file_path)
        ...
        for page_file in page_files:
            ...
            if 'modelView' in data and 'components' in data['modelView']:
                check_components_for_sensitive_comments(data['modelView']['components'], page_file)